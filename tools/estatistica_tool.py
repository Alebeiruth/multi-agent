...
TOTAL_AULAS = 8
EMAIL_ALEXANDRE = os.getenv("ALEXANDRE_EMAIL", "alexandre@email.com")
...
def marcar_revisao_feita(numero_aula: int, numero_revisao: int) -> str:
    ...
    """
    try:
        RevisaoInput(numero_aula=numero_aula, numero_revisao=numero_revisao)
    except ValidationError as e:
        return f"Erro de validação: {e.errors()[0]['msg']}"
    ...
    feitas = aula.setdefault("revisoes_feitas", [])
