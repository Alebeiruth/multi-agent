...
def registrar_modulo_datacamp(
    ...
    feitos_trilha = {
        m["modulo"] for m in data["modulos_datacamp"] if m["curso"] == curso
    }
...
def consultar_progresso_aws_datacamp() -> str:
    ...
    linhas.append(f"🏗️  AWS Solutions Architect Associate")
    ...
    for trilha, modulos_roadmap in ROADMAP_DATACAMP.items():
        feitos_trilha = {
            m["modulo"] for m in modulos_dc if m["curso"] == trilha
        }
...
def sugerir_proximo_aws_datacamp(plataforma: str = "") -> str:
    ...
    if plataforma == "aws":
        ...
        feitos_trilha = {
            m["modulo"] for m in data.get("modulos_datacamp", [])
            if m["curso"] == trilha_menor
        }
