...
def consultar_progresso_computacao() -> str:
    ...
    if not estudos:
        return (
            "Nenhum estudo registrado ainda. Comece registrando um tópico com registrar_estudo_cs."
        )
    ...
    linhas = ["💻 Progresso — Computação & Agentes\n"]
    ...
    for area, topicos_roadmap in ROADMAP.items():
        ...
        if pendentes:
            linhas.append(
                f"  ⬜ Próximos: {', '.join(pendentes[:3])}{'...' if len(pendentes) > 3 else ''}"
            )
...
def registrar_duvida(
    ...
    return f"📝 Dúvida registrada: '{topico}'\nDúvidas pendentes: {pendentes}"
